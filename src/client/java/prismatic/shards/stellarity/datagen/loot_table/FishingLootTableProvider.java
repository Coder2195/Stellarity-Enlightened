...
public class FishingLootTableProvider extends SimpleFabricLootTableSubProvider {
	...
	public void generate(@NonNull BiConsumer<ResourceKey<LootTable>, LootTable.Builder> consumer) {
		...
		consumer.accept(VOID_FISHING_FISH, lootTable().withPool(pool()
			.add(item(ENDER_KOI).setWeight(15).apply(count(num(2, 4))))
		...
		consumer.accept(VOID_FISHING_JUNK, lootTable().withPool(pool()
			.add(item(CRYING_OBSIDIAN).apply(count(num(1, 3))))
			.add(item(OBSIDIAN).apply(count(num(1, 4))))
			.add(item(END_STONE).apply(count(num(3, 9))))
			.add(item(CHORUS_FRUIT).apply(count(num(1, 5))))
			.add(item(POPPED_CHORUS_FRUIT).setWeight(2).apply(count(num(2, 5))))
			.add(item(PURPUR_BLOCK).apply(count(num(6, 12))))
			.add(item(PHANTOM_MEMBRANE).setWeight(3).apply(count(num(1, 3))))
			.add(item(ENDER_PEARL).setWeight(3).apply(count(num(1, 2))))
			.add(item(SHULKER_SHELL).setWeight(5).apply(count(num(1, 2))))
		...
		consumer.accept(VOID_FISHING_TREASURE, lootTable().withPool(pool()
			.add(item(EYE_ARMOR_TRIM_SMITHING_TEMPLATE).setWeight(7).apply(count(num(1, 2))))
			.add(item(SPIRE_ARMOR_TRIM_SMITHING_TEMPLATE).setWeight(7).apply(count(num(1, 2))))
			.add(item(END_CRYSTAL).setWeight(5).apply(count(num(1, 2))))
			.add(item(END_CRYSTAL).setWeight(3).apply(count(num(2, 3))))
		...
		consumer.accept(VOID_FISHING_LOCATION_AMETHYST_BIOMES, lootTable().withPool(pool()
			...
			.add(item(AMETHYST_SHARD).setWeight(5).apply(count(num(2, 4))))
			.add(item(QUARTZ).setWeight(5).apply(count(num(3, 6))))
			.add(item(AMETHYST_BLOCK).setWeight(3).apply(count(num(2, 3))))
		...
		consumer.accept(VOID_FISHING_LOCATION_FIERY_HILLS, lootTable().withPool(pool()
			...
			.add(item(BLAZE_ROD).setWeight(3).apply(count(num(1, 2))))
			.add(item(MAGMA_BLOCK).setWeight(6).apply(count(num(2, 4))))
			.add(item(GLOWSTONE_DUST).setWeight(7).apply(count(num(3, 7))))
			.add(item(GHAST_TEAR))
		));
		...
		consumer.accept(VOID_FISHING_LOCATION_FLESH_TUNDRA, lootTable().withPool(pool()
			...
			.add(item(ROTTEN_FLESH).setWeight(16).apply(count(num(3, 6))))
			.add(item(NETHER_WART_BLOCK).setWeight(12).apply(count(num(2, 3))))
			.add(item(GUNPOWDER).setWeight(12).apply(count(num(1, 5))))
			.add(item(BONE).setWeight(14).apply(count(num(1, 4))))
		));

		consumer.accept(VOID_FISHING_LOCATION_FROZEN_SPIKES, lootTable().withPool(pool()
			.add(item(FROST_MINNOW).setWeight(7))
			.add(item(ICE).apply(count(num(2, 4))))
			.add(item(PACKED_ICE).apply(count(num(2, 4))))
			.add(item(BLUE_ICE).apply(count(num(2, 3))))
			.add(item(SNOW_BLOCK).apply(count(num(2, 4))))
			.add(item(SNOWBALL).apply(count(num(3, 8))))
		));

		consumer.accept(VOID_FISHING_LOCATION_VANILLA_BIOMES, lootTable().withPool(pool()
			.add(item(OVERGROWN_COD).setWeight(8))
			.add(item(CHORUS_FLOWER).setWeight(3).apply(count(num(1, 2))))
			.add(item(ENDER_EYE).setWeight(2).apply(count(num(1, 2))))
			.add(item(CHORUS_PLANT).setWeight(4).apply(count(num(3, 8))))
		));

		consumer.accept(VOID_FISHING_LOCATION_PRISMARINE_FOREST, lootTable().withPool(pool()
			.add(item(BUBBLEFISH).setWeight(12))
			.add(item(PRISMARINE).setWeight(6).apply(count(num(3, 5))))
			.add(item(PRISMARINE_CRYSTALS).setWeight(5).apply(count(num(2, 6))))
			.add(item(GLOW_INK_SAC).setWeight(5).apply(count(num(1, 2))))
		));

		consumer.accept(VOID_FISHING_LOCATION_THE_HALLOW, lootTable().withPool(pool()
			.add(item(PRISMITE).setWeight(25))
			.add(item(STARLIGHT_SOOT).setWeight(9).apply(count(num(2, 4))))
			.add(item(CHERRY_LEAVES).setWeight(6).apply(count(num(2, 5))))
			.add(item(PINK_PETALS).setWeight(6).apply(count(num(3, 6))))
		));

		consumer.accept(VOID_FISHING_LOCATION_WARPED_MARSH, lootTable().withPool(pool()
			.add(item(GOOSH).setWeight(20))
			.add(item(MOSS_BLOCK).setWeight(3).apply(count(num(3, 5))))
